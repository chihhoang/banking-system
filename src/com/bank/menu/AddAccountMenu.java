...
public class AddAccountMenu extends javax.swing.JDialog {
	...
	private void okButtonActionPerformed(java.awt.event.ActionEvent evt) {// GEN-FIRST:event_okButtonActionPerformed
		...
		if (!ssnField.getText().matches("\\d{3}-?\\d{2}-?\\d{4}")) {
			warnings.append(
			    "Social security number is not valid. (Format: 123-45-6789)\n");
		} else {
		...
		if (depositField.getText().isEmpty()) {
			...
			try {
				initialDeposit = Bank.round(Double.parseDouble(depositField.getText()),
				    2);
		...
		if (warnings.length() > 0) {
			JOptionPane.showMessageDialog(this, warnings.toString(), "Input Error",
			...
			if (accountType != AccountType.Undefined) {
				int accountId = bank.openAccount(firstName, lastName, ssn, accountType, initialDeposit);
